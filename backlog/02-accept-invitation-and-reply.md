# 02 — Accept an invitation and create a reply

## Purpose

Let a friend open a host's invitation link, prepare their local side of the match, and obtain a reply link to return to the host.

## Acceptance criteria

- Opening a valid invitation link clearly identifies that the visitor is joining a game.
- The application prepares a local match record for the friend.
- The application presents a shareable reply link.
- The friend can copy the reply link and send it outside the application.
- The application clearly explains that the host must open the reply link next.
